...
class AttendanceReport(ReportXlsx):
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        holiday_regular = workbook.add_format({'align':'center','bold':False,'text_wrap': True,'bg_color': '#FFC7CE',})
        regular = workbook.add_format({'align':'center','bold':False,'text_wrap': True})
        attend_regular = workbook.add_format({'align':'center','bold':False,'font_color': 'brown','text_wrap': True})
        ...
        for emp in active_ids:
            ...
            print "carryyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy", carry_forward
            sunday_days = emp.get_sunday(emp.id, inv[0].from_date, inv[0].to_date)
            for abs_att in self.env['hiworth.hr.attendance'].search([('name', '=', emp.id), ('date', '>=', inv[0].from_date),('date','<=',inv[0].to_date),('attendance','in',['absent','half'])],order='date asc'):
                            ...
                            if abs_att.attendance == 'half':
                                ...
                                if carry_forward < 1:
                                    ...
                                    leave_type = self.env['hr.holidays.status'].search([('limit', '=', False)], limit=1,
                                                                                       order='id asc')
                                    config_employee = self.env['employee.config.leave'].search([('employee_id', '=', emp.id),
                                                                                                (
                                                                                                'leave_id', '=', leave_type.id),
                                                                                                ])
                                    if config_employee:
                                    ...
                                    if month_leave:
                                        if month_leave.exgratia >= month_leave.remaining:
                                            ...
                                            exgratia = (len(exgratia_full_half_ids) * .5) + (len(exgratia_half_ids) * .5)
                                    ...
                                    if month_leave:
                                        if month_leave.exgratia >= month_leave.remaining:
                                            ...
                                            exgratia = len(exgratia_full_ids) + (len(exgratia_half_ids) * 0.5) + (len(exgratia_full_half_ids) * 0.5)
            ...
            print "carry forwardhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh", carry_forward
            ...
            print "hhhhhhhhhhhhh",emp.joining_date
            join_date = ''
            ...
            for i in range(day_last_day):
                if day_first_day < 23:
                    ...
                    if attendance.attendance == 'full':
                        if not attendance.compensatory_off:
                            ...
                            if attendance.half_compensatory_off:
                                att = .5
                        attend_regular = workbook.add_format({'align': 'center', 'bold': False, 'font_color': 'green','text_wrap': True})
                    ...
                    if attendance.attendance == 'absent':
                        attendan = 'AB'
                        no_absent_days += 1
                        attend_regular = workbook.add_format({'align': 'center', 'bold': False, 'font_color': 'red','text_wrap': True})
                    ...
                    if not public_holiday and day_first.weekday()!=6:
                        ...
                        print "hhhhhhhhhhhhhhhhhhhhhhhhhh", sunday_attendance
