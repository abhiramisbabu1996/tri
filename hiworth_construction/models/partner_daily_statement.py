...
class PartnerDailyStatement(models.Model):
	...
	def cancel_entry(self):
		...
		# for record in self.products_used_lines:
		# 	if record.product_id and record.used_qty:
		# 		location_id = record.partner_id.location_ids
		# 		current_qty=sum(self.env['stock.quant'].search([('product_id', '=', record.product_id.id),
		# 										('location_id', '=', location_id.id)]).mapped('qty'))
		# 		new_quantity = current_qty + record.used_qty
		# 		if new_quantity<0:
		# 			raise osv.except_osv('Warning!', ('%s stock quantity is less than used quantity', (record.product_id.name)))
		# 		else:
		# 			update_qty = self.env['stock.change.product.qty'].create({
		# 				'location_id': location_id.id,
		# 				'new_quantity': new_quantity,
		# 				'product_id': record.product_id.id
		# 			})
		# 			update_qty.change_product_qty()
...
class PartnerDailyStatementLine(models.Model):
	...
	task_id = fields.Many2one("project.task")
	estimation_line_id = fields.Many2one('task.details')
...
class ReceivedProducts(models.Model):
	...
	def onchange_product_id(self):
		for record in self:
			...
			if record.product_id and record.partner_id.location_ids:
				current_qty = sum(self.env['stock.quant'].search([('product_id', '=', record.product_id.id),
																  ('location_id', '=', record.partner_id.location_ids.id)]).mapped('qty'))
	...
	# def onchange_goods_receive_report_id(self):
	# 	for rec in self:
	# 		rec.po_no = rec.goods_receive_report_id.purchase_id
			# rec.product_id = rec.goods_receive_report_line_id.item_id
			# rec.po_qty = rec.goods_receive_report_line_id.po_quantity
			# rec.received_qty = rec.goods_receive_report_line_id.quantity_accept
			# rec.vehicle_id = rec.goods_receive_report_id.vehicle_id

	# @api.depends('goods_receive_report_line_id')
